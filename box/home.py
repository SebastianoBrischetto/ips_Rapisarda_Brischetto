    ...
    try:
        ...
        while o == -1:
            try:
                ...
                display.scrivi("Loading...", "", "Connessione", "in corso....", "")
                time.sleep(100)
                a += 1
                if a > 4:
        ...
        # Display device info
        display.scrivi("IPS", "", "Dispositivo in", "caricamento", f"{iip}:443")
        time.sleep(0.4)
        ...
        nssid = os.popen("sudo iwgetid -r").read()
        display.scrivi("IPS", "", "Dispositivo Pronto", "WiFi: " + nssid, f"{iip}:443")
...
def setup_routes(app):
    ...
    app.add_url_rule("/config", "config", configtemp)
    app.add_url_rule("/live", "live", live)
    app.add_url_rule("/daticonfig", "daticoonfig", daticoonfig)
    app.add_url_rule("/config", "config", config, methods=["POST", "GET"])
    app.add_url_rule("/sm", "sm", sm)
    app.add_url_rule("/termina_calibrazione", "termina_calibrazione", termina_calibrazione)
    app.add_url_rule("/nuova_calibrazione", "show_nuova_calibrazione", show_nuova_calibrazione)
    app.add_url_rule("/nuova_calibrazione", "nuova_calibrazione", nuova_calibrazione, methods=["POST"])
    app.add_url_rule("/view_calibration_plot", "view_calibration_plot", view_calibration_plot, methods=["POST"])
    app.add_url_rule("/cali", "cali", cali)
    app.add_url_rule("/cali", "calibra", calibra, methods=["POST", "GET"])
    app.add_url_rule("/meas", "meas", meas, methods=["GET"])
    app.add_url_rule("/misu", "misu", misu)
    app.add_url_rule("/end", "end", end)
    app.add_url_rule("/down_pac", "down_pac", down_pac)
    app.add_url_rule("/up", "up", up)
    app.add_url_rule("/pagend", "pagend", pagend)
    app.add_url_rule("/remove_measure", "remove_measure", remove_measure, methods=["GET"])
    app.add_url_rule("/dati", "datas", datas)
    ...
    app.add_url_rule("/points", "points", lambda: render_template('points/index.html'))
    app.add_url_rule("/addpoints", "add_points", lambda: render_template('points/addpoints.html'))
    app.add_url_rule("/map", "maps", lambda: render_template('points/map.html'))


def run_application(iip):
